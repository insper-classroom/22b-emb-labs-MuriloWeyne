...
SemaphoreHandle_t xSemaphoreButLed1;

/** Queue for msg log send data */
QueueHandle_t xQueueLedFreq;
QueueHandle_t xQueueButLed;
...
void but_callback(void) {
  ...
  int increment = 100;
  xQueueSendFromISR(xQueueButLed, (void *) &increment, xHigherPriorityTaskWoken);
}
...
static void task_but(void *pvParameters) {
  ...
  for (;;) {
    ...
    if(xQueueReceive(xQueueButLed, &msg, (TickType_t) 0)) {
      delayTicks += msg;
      if(delayTicks < 100) {
        delayTicks = 900;
      }
      else if(delayTicks > 1000) {
        delayTicks = 900;
      }
      printf("task_but %d \n", delayTicks);
...
static void BUT_init(void) {
  ...
  NVIC_SetPriority(BUT_LED_1_ID, 5); // Prioridade 3
}
...
int main(void) {
  ...
  xSemaphoreButLed1 = xSemaphoreCreateBinary();

  if (xSemaphoreBut == NULL)
    printf("falha em criar o semaforo \n");

  if (xSemaphoreButLed1 == NULL) {
    printf("falha em criar o semáforo led1 \n");
  }

  /* cria queue com 32 "espacos" */
  /* cada espaço possui o tamanho de um inteiro*/
  xQueueButLed = xQueueCreate(32, sizeof(int));
  if(xQueueButLed == NULL) {
    printf("Falha ao criar a queue but \n");
  }
