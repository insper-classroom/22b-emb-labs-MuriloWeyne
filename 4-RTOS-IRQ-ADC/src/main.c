...
static void AFEC_pot_callback(void) {
  ...
  xQueueSendFromISR(xQueueProcessedData, &adc, &xHigherPriorityTaskWoken);
}
...
static void task_proc(void *pvParameters) {
  ...
  while (1) {
    int avg = 0;
    int sum = 0;
    if (xQueueReceive(xQueueProcessedData, &(adc), 1000)) {
      adcs[i] = adc.value;
      for (int j = 0; j < 10; j++) {
        sum += adcs[j];
      }
      ...
      if (i == 10) {
        i = 0;
      } // Zera o contador para remover o adc mais antigo
...
int main(void) {
  ...
  xQueueBruteData = xQueueCreate(100, sizeof(adcData));
  if (xQueueBruteData == NULL)
    printf("falha em criar a queue xQueueBruteData \n");
  ...
  xQueueProcData = xQueueCreate(100, sizeof(int));
  if (xQueueProcData == NULL)
    printf("falha em criar a queue xQueueProcData \n");
